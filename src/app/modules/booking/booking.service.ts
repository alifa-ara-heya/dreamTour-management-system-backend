...
const createBooking = async (payload: Partial<IBooking>, userId: string) => {
    ...
    const session = await Booking.startSession(); //Starts a MongoDB session for benefits like causal consistency, retryable writes, and transactions.
    ...
    try {
        ...
        if (!user?.phone || !user?.address) {
            throw new AppError(httpStatus.BAD_REQUEST, "Please Update Your Profile (Add Phone number and your address ) To Book a Tour")
        }
